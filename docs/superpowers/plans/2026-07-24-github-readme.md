...
**Tech Stack:** GitHub-flavored Markdown, Mermaid, PNG screenshots from the Streamlit workbench, Python pytest contracts with Pillow image verification (Pillow is already supplied by the declared Streamlit stack), Codex in-app browser capture.
...
- Create `tests/test_readme.py`: stable contract for required claims, URLs, commands, status flow, prohibited release language, and complete decodable screenshot assets.
